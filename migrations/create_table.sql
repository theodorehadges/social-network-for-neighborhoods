...
create table userm(id serial primary key , firstname varchar, lastname varchar, email varchar, street varchar, city varchar, state char(2), zipcode int,
description varchar, photo varchar, lat float, long float, block_id int references block(id), created_on timestamp);
create table profile(id serial primary key, description varchar, photo varchar,lat float, long float, user_id int references userm(id) unique, created_on timestamp);
create table friend(id serial primary key , user_1_id int references userm(id), user_2_id int references userm(id), created_on timestamp);
